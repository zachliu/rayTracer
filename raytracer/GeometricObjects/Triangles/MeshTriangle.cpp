...
// ---------------------------------------------------------------- constructor
// the normal is computed in Grid::read_obj_file
...
MeshTriangle::get_bounding_box(void) {	
	...
	return(BBox(std::min(std::min(v1.x, v2.x), v3.x) - delta, std::max(std::max(v1.x, v2.x), v3.x) + delta, 
...
double 
MeshTriangle::interpolate_u(const double beta, const double gamma) const {	
...
double 
MeshTriangle::interpolate_v(const double beta, const double gamma) const {	
