...
Sampler::Sampler(void)						
	...
	ray_flag.reserve(N*N);
	ray_color.reserve(N*N);
}
...
// ------------------------------------------------------------------ constructor
Sampler::Sampler(const int ns, const double fdiff)
	: 	num_samples(ns),
		num_sets(83),
		sub_division_level(ns),
		minimum_size(.5),
...
Sampler::Sampler(const Sampler& s)				
	: 	num_samples(s.num_samples),
		num_sets(s.num_sets),
		sub_division_level(s.sub_division_level),
...
Sampler::operator= (const Sampler& rhs)	{
	...
	samples				= rhs.samples;
	sub_division_level  = rhs.sub_division_level;
	minimum_size		= rhs.minimum_size;
...
void
Sampler::map_samples_to_hemisphere(const double exp) {
	size_t size = samples.size();
